    ...
    public function delsystemuser()
    {
        ...
        foreach ($users as $user) {
            ...
            if ($this->action == 'Delete') {
                ...
                $userTradingBots = UserTradingBot::where('user_id', $user->id)->get();
                ...
                $copySubscriptions = CopySubscription::where('user_id', $user->id)->get();
            ...
            if ($this->action == 'Clear') {
                ...
                // Delete user's trading bot records when clearing account
                $userTradingBots = UserTradingBot::where('user_id', $user->id)->get();
                if (!empty($userTradingBots)) {
                    foreach ($userTradingBots as $userBot) {
                        // Delete trading logs for this bot
                ...
                // Delete user's copy trading records when clearing account
                $copySubscriptions = CopySubscription::where('user_id', $user->id)->get();
                if (!empty($copySubscriptions)) {
                    foreach ($copySubscriptions as $subscription) {
                        // Delete copy trades for this subscription
