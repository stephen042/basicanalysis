...
class ViewsController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $portfolioData = $this->calculatePortfolioPerformance($user->id);
        ...
        $topExperts = ExpertTrader::where('status', 'active')
            ->orderByDesc('roi_percentage')
            ->take(3)
            ->get();

        // Get active copy trading subscriptions
        $activeCopySubscriptions = CopySubscription::where('user_id', Auth::user()->id)
            ->where('status', 'active')
            ->with('expertTrader')
            ->orderByDesc('id')
            ->get();
        ...
        return view('user.dashboard', [
            ...
            'total_balance' => $user->account_bal + $user->roi,
            'total_withdrawal' => $total_withdrawal,
            'portfolioData' => $portfolioData,
            'trading_accounts' => Mt4Details::where('client_id', Auth::user()->id)->count(),
            'plans' => UserTradingBot::where('user_id', Auth::user()->id)->where('status', 'active')->with('tradingBot')->orderByDesc('id')->get(),
            'plan_stat' => [
                'active' => UserTradingBot::where('user_id', Auth::user()->id)->where('status', 'active')->count()
            ],
            't_history' => Tp_Transaction::where('user', Auth::user()->id)
                ->where('type', '<>', 'ROI')
                ->orderByDesc('id')->skip(0)->take(10)
                ->get(),
            'recent_deposits' => Tp_Transaction::where('user', Auth::user()->id)
                ->orderByDesc('id')->skip(0)->take(10)
                ->get(),
...
if (count($words) < 12) {
     ...
     * Calculate portfolio performance data for the last 12 months
     */
    private function calculatePortfolioPerformance($userId)
    {
        $months = [];
        $values = [];
        $runningBalance = 0;
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            $monthlyDeposits = Deposit::where('user', $userId)
                ->where('status', 'Processed')
                ->whereBetween('created_at', [$monthStart, $monthEnd])
                ->sum('amount');

            // Calculate withdrawals for this month
            $monthlyWithdrawals = Withdrawal::where('user', $userId)
                ->where('status', 'Processed')
                ->whereBetween('created_at', [$monthStart, $monthEnd])
                ->sum('amount');
            ...
            // Calculate ROI/Investment profits for this month
            $monthlyROI = Tp_Transaction::where('user', $userId)
                ->where('type', 'ROI')
                ->whereBetween('created_at', [$monthStart, $monthEnd])
                ->sum('amount');

            // Update running balance
            $runningBalance += ($monthlyDeposits - $monthlyWithdrawals + $monthlyTradingPnL + $monthlyROI);

            // Ensure minimum value is 0
            $values[] = max(0, $runningBalance);
        }
