                ...
                <div class="mb-5 row">
                    <div class="col-lg-8 offset-lg-2 card p-md-4 p-2  shadow">
                        ...
                        <div class="alert alert-danger">
                            ...
                            <p>The user associated with this withdrawal request has been deleted from the system.</p>
                            <hr>
                            <div class="mb-3">
                                <strong>Withdrawal ID:</strong> {{ $withdrawal->id }}<br>
                                <strong>Amount:</strong> {{ $settings->currency }}{{ number_format($withdrawal->amount, 2) }}<br>
                                <strong>Amount + Charges:</strong> {{ $settings->currency }}{{ number_format($withdrawal->to_deduct, 2) }}<br>
                                <strong>Payment Mode:</strong> {{ $withdrawal->payment_mode }}<br>
                                <strong>Status:</strong> <span class="badge badge-{{ $withdrawal->status == 'Processed' ? 'success' : 'danger' }}">{{ $withdrawal->status }}</span><br>
                                <strong>Date:</strong> {{ \Carbon\Carbon::parse($withdrawal->created_at)->toDayDateTimeString() }}
                            </div>
                            <p class="text-muted"><small><i class="fa fa-info-circle"></i> This withdrawal record cannot be processed because the user account no longer exists.</small></p>
                        </div>
                        @endif
                    </div>
                </div>
