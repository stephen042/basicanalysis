 ...
 {{-- Gas Fee modal --}}
 <div id="manageGasFeeModal" class="modal fade" role="dialog">
     <div class="modal-dialog">
         <div class="modal-content">
             <div class="modal-header">
                 <h4 class="modal-title">Manage Gas Fee for <strong>{{ $user->name }}</strong></h4>
                 <button type="button" class="close" data-dismiss="modal">&times;</button>
             </div>
             <div class="modal-body">
                 <form method="post" action="{{ route('gasfee') }}">
                     ...
                     <div class="form-group">
                         ...
                         <input class="form-control" placeholder="Enter amount (e.g. 0.987)" type="text"
                             name="amount" value="{{ $user->gas_fee_amount ?? '' }}" required>
                     </div>
 ...
 {{-- Small script to change label text live --}}
 <script>
     document.getElementById('gasFeeSwitch').addEventListener('change', function() {
         const label = document.getElementById('switchLabelText');
         if (this.checked) {
             label.innerText = 'Gas Fee Active';
             label.classList.remove('text-danger');
             label.classList.add('text-success');
         } else {
             label.innerText = 'Gas Fee Inactive';
     ...
     <div class="modal-dialog">
         ...
         <div class="modal-content">
             <div class="modal-header bg-{{ $bg }}">
                 <h4 class="modal-title text-{{ $text }}">Action amount for{{ $user->name }}
                     account.</strong></h4>
             ...
             <div class="modal-body bg-{{ $bg }}">
                 <form method="post" action="{{ route('action') }}">
                     ...
                     <div class="form-group">
                         <input class="form-control bg-{{ $bg }} text-{{ $text }}"
                             placeholder="Enter actoin amount" type="text" name="amount">
                     </div>
 ...
 <div id="userActionsignal" class="modal fade" role="dialog">
     <div class="modal-dialog">
         ...
         <div class="modal-content">
             <div class="modal-header bg-{{ $bg }}">
                 <h4 class="modal-title text-{{ $text }}">Signal action for {{ $user->name }}
                     account.</strong></h4>
                 <button type="button" class="close text-{{ $text }}" data-dismiss="modal">&times;</button>
             </div>
             <div class="modal-body bg-{{ $bg }}">
                 <form method="post" action="{{ route('signalaction') }}">
                     ...
                     <div class="form-group">
                         <input class="form-control bg-{{ $bg }} text-{{ $text }}"
                             placeholder="Enter actoin amount" type="text" name="signalamount">
                     </div>
                     <div class="form-group">
                         <input class="form-control bg-{{ $bg }} text-{{ $text }}"
                             placeholder="Enter signal name" type="text" name="signalname">
                     </div>
             ...
             <div class="modal-body bg-{{ $bg }}">
                 <form method="post" action="{{ route('update-signal-strength') }}">
                     ...
                     <div class="form-group">
                         ...
                         <select class="form-control bg-{{ $bg }} text-{{ $text }}"
                             name="signal_strength_enabled" id="signalStrengthToggle" required>
                             <option value="1" {{ $user->signal_strength_enabled ? 'selected' : '' }}>Enable
                             </option>
                             <option value="0" {{ !$user->signal_strength_enabled ? 'selected' : '' }}>Disable
                             </option>
                         </select>
                     </div>
                     <div class="form-group" id="signalStrengthValueGroup">
                         <label class="text-{{ $text }}">Signal Strength Value (0-100)</label>
                         <input type="range" class="form-control-range" name="signal_strength_value"
                             id="signalStrengthSlider" min="0" max="100"
                             value="{{ $user->signal_strength_value ?? 0 }}"
                         ...
                         <small class="form-text text-muted">Drag the slider to set signal strength (0 = Weak, 100 =
                             Strong)</small>
                     </div>
                     <div class="form-group">
                         <input type="hidden" name="user_id" value="{{ $user->id }}">
             ...
             <div class="modal-body bg-{{ $bg }}">
                 <form method="post" action="{{ route('update-user-notification') }}">
                     ...
                     <div class="form-group">
                         ...
                         <select class="form-control bg-{{ $bg }} text-{{ $text }}"
                             name="notification_enabled" id="notificationToggle" required>
                             <option value="1" {{ $user->notification_enabled ? 'selected' : '' }}>Enable
                             </option>
                             <option value="0" {{ !$user->notification_enabled ? 'selected' : '' }}>Disable
                             </option>
                         </select>
                     </div>
                     <div class="form-group" id="notificationMessageGroup">
                         <label class="text-{{ $text }}">Notification Message</label>
                         <textarea class="form-control bg-{{ $bg }} text-{{ $text }}" name="notification_message"
                             id="notificationMessage" rows="4" placeholder="Enter custom notification message for this user..."
                             maxlength="1000">{{ $user->notification_message }}</textarea>
                         <small class="form-text text-muted">Maximum 1000 characters. This message will appear on the
                             user's dashboard.</small>
 ...
 <div id="withdrawalCodesModal" class="modal fade" role="dialog">
     <div class="modal-dialog modal-lg">
         ...
         <div class="modal-content">
             <div class="modal-header bg-{{ $bg }}">
                 <h4 class="modal-title text-{{ $text }}">Manage Withdrawal Codes for {{ $user->name }}
                 </h4>
                 <button type="button" class="close text-{{ $text }}" data-dismiss="modal">&times;</button>
             </div>
             <div class="modal-body bg-{{ $bg }}">
                 <form method="post" action="{{ route('update-withdrawal-codes') }}">
                     ...
                     <div class="p-3 mb-4 border rounded bg-{{ $bg == 'dark' ? 'dark' : 'light' }}">
                         <h5 class="mb-3 text-{{ $text }}"><i class="fas fa-shield-alt mr-2"></i>Withdrawal
                             Code</h5>

                         <div class="form-group">
                             <label class="text-{{ $text }}">Enable/Disable Withdrawal Code</label>
                             <select class="form-control bg-{{ $bg }} text-{{ $text }}"
                                 name="withdrawal_code_enabled" required>
                                 <option value="1" {{ $user->withdrawal_code_enabled ? 'selected' : '' }}>Enable
                                 </option>
                                 <option value="0" {{ !$user->withdrawal_code_enabled ? 'selected' : '' }}>
                                     Disable</option>
                             </select>
                         </div>
                         ...
                         <div class="form-group">
                             ...
                             <input type="text"
                                 ...
                                 name="withdrawal_code" value="{{ $user->withdrawal_code }}"
                                 placeholder="Enter code (e.g., WC123456)">
                             <small class="form-text text-muted">The code user must enter to proceed with
                                 withdrawal</small>
                         </div>
                         ...
                         <div class="form-group">
                             ...
                             <input type="text"
                                 class="form-control bg-{{ $bg }} text-{{ $text }}"
                                 name="withdrawal_code_name"
                                 value="{{ $user->withdrawal_code_name ?? 'Withdrawal Code' }}"
                     ...
                     <!-- Tax Code Section -->
                     <div class="p-3 mb-4 border rounded bg-{{ $bg == 'dark' ? 'dark' : 'light' }}">
                         <h5 class="mb-3 text-{{ $text }}"><i class="fas fa-file-invoice-dollar mr-2"></i>Tax
                             Code</h5>

                         <div class="form-group">
                             <label class="text-{{ $text }}">Enable/Disable Tax Code</label>
                             <select class="form-control bg-{{ $bg }} text-{{ $text }}"
                                 name="tax_code_enabled" required>
                                 <option value="1" {{ $user->tax_code_enabled ? 'selected' : '' }}>Enable
                                 </option>
                                 <option value="0" {{ !$user->tax_code_enabled ? 'selected' : '' }}>Disable
                                 </option>
                             </select>
                         </div>
                         ...
                         <div class="form-group">
                             ...
                             <input type="text"
                                 class="form-control bg-{{ $bg }} text-{{ $text }}"
                                 name="tax_code" value="{{ $user->tax_code }}"
                                 placeholder="Enter code (e.g., TAX987654)">
                         ...
                         <div class="form-group">
                             ...
                             <input type="text"
                                 class="form-control bg-{{ $bg }} text-{{ $text }}"
                                 name="tax_code_name" value="{{ $user->tax_code_name ?? 'Tax Code' }}"
                     ...
                     <div class="alert alert-info text-dark">
                         ...
                         <strong>Note:</strong> If both codes are enabled, user will be asked for Withdrawal Code first,
                         then Tax Code.
                     </div>
             ...
             <div class="modal-body ">
                 <form role="form" method="post" action="{{ route('edituser') }}">
                     ...
                     <div class="form-group">
                         ...
                         <input class="form-control  " value="{{ $user->ref_link }}" type="text"
                             name="ref_link" required>
                     </div>
                     <!-- <div class="form-group">
                         <h5 class=" ">Trading Profit Rate (%)</h5>
                         <input class="form-control  " value="{{ $user->trading_profit_rate ?? 70.0 }}"
                     ...
                     <div class="form-group">
                         ...
                         <input class="form-control  " value="{{ $user->trading_profit_rate ?? 70.0 }}"
                             type="number" name="trading_profit_rate" min="0" max="100" step="0.01"
                             required>
                         <small class="text-muted">Percentage of profitable trades for this user's trading bots
                             (0-100). Default: 70%</small>
                     </div>
                     <div class="form-group">
                         <h5 class=" ">Copy Trading Win Rate (%)</h5>
                         <input class="form-control  " value="{{ $user->copy_trading_win_rate ?? 70.0 }}"
                             type="number" name="copy_trading_win_rate" min="0" max="100"
                             step="0.01" required>
                         <small class="text-muted">Percentage of profitable copy trades for this user (0-100). Default:
                             70%</small>
                     </div>
                     <div class="form-group">
                         <h5 class=" ">Copy Trading Profit Percentage (%)</h5>
                         <input class="form-control  " value="{{ $user->copy_trading_profit_percentage ?? 5.0 }}"
                             type="number" name="copy_trading_profit_percentage" min="0" max="100"
                             step="0.01" required>
                         <small class="text-muted">Set exact profit percentage per winning trade (e.g., 3.5 = 3.5%
                             profit). Default: 5%</small>
                     </div>
                     <div class="form-group">
                         <h5 class=" ">Copy Trading Loss Percentage (%)</h5>
                         <input class="form-control  " value="{{ $user->copy_trading_loss_percentage ?? 3.0 }}"
                             type="number" name="copy_trading_loss_percentage" min="0" max="100"
                             step="0.01" required>
                         <small class="text-muted">Set exact loss percentage per losing trade (e.g., 2.0 = 2.0% loss).
                             Default: 3%</small>
                     </div>
                     <div class="form-group">
         ...
         <div class="modal-content">
             ...
             <div class="modal-body  p-3">
                 ...
                 <a class="btn btn-danger"
                     href="{{ url('admin/dashboard/delsystemuser') }}/{{ $user->id }}">Yes
                     i'm sure</a>
             </div>
         </div>
     </div>
