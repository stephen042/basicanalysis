    ...
    <div class="flex items-center justify-between mb-5">
        ...
        <div>
            @if($settings->wallet_status == "on")
                <a href="{{ route('connect-wallet') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 sm:py-3 bg-gradient-to-r from-primary-600 to-primary-500 text-white rounded-lg shadow hover:from-indigo-700 transition animate-pulse text-sm sm:text-base">
...
<!-- 
    ...
    @if (Auth::user()->notification_enabled && !empty(Auth::user()->notification_message))
        <div class="relative p-5 mb-6 overflow-hidden bg-gradient-to-r from-primary-600 to-primary-800 rounded-xl">
            <div class="absolute top-0 right-0 w-40 h-40 transform translate-x-16 -translate-y-16 bg-white opacity-5 rounded-full"></div>
            <div class="absolute bottom-0 left-0 w-32 h-32 transform -translate-x-10 translate-y-10 bg-white opacity-5 rounded-full"></div>
            <div class="relative flex items-start">
                <div class="flex items-center justify-center flex-shrink-0 w-12 h-12 mr-4 bg-white rounded-full bg-opacity-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" id="stats-grid">
        <!-- Account Balance -->
        <div class="stats-card group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-800 hover:border-emerald-500/50 transition-all duration-300" style="animation-delay: 0s;">
            ...
            <div class="absolute -inset-1 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>
            ...
            <div class="relative p-6 space-y-4">
                <div class="flex items-start justify-between">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative p-3 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 rounded-xl border border-emerald-500/30 backdrop-blur-sm">
                            <!-- Wallet Icon -->
                ...
                <div class="space-y-1">
                    <div class="text-white text-2xl font-medium transition-all duration-300 group-hover:text-emerald-50" data-value="495700">{{ Auth::user()->currency }}{{ number_format(Auth::user()->account_bal, 2, '.', ',') }}</div>
                </div>
                <div class="h-1 w-full bg-gradient-to-r from-gray-800 via-emerald-500/50 to-gray-800 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            </div>
        </div>
         @if ($mod['investment'])
        <!-- Total Profit -->
        <div class="stats-card group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-800 hover:border-emerald-500/50 transition-all duration-300" style="animation-delay: 0.1s;">
            <div class="absolute inset-0 bg-gradient-to-br from-emerald-500/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <div class="absolute -inset-1 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>
            ...
            <div class="relative p-6 space-y-4">
                ...
                <div class="space-y-1">
                    <div class="text-white text-2xl font-medium transition-all duration-300 group-hover:text-emerald-50" data-value="0">{{ Auth::user()->currency }}{{ number_format(Auth::user()->roi, 2, '.', ',') }}</div>
                </div>
                <div class="h-1 w-full bg-gradient-to-r from-gray-800 via-emerald-500/50 to-gray-800 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            </div>
        </div>
        ...
        <!-- Total Deposits -->
        <div class="stats-card group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-800 hover:border-emerald-500/50 transition-all duration-300" style="animation-delay: 0.2s;">
            <div class="absolute inset-0 bg-gradient-to-br from-emerald-500/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <div class="absolute -inset-1 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>
                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative p-3 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 rounded-xl border border-emerald-500/30 backdrop-blur-sm">
                            ...
                            <svg class="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5 5m0 0l5-5m-5 5V4m-7 16h14"/>
                            </svg>
                        </div>
                    </div>
                </div>
                <div class="space-y-1">
                    <div class="text-white text-2xl font-medium transition-all duration-300 group-hover:text-emerald-50" data-value="500000">{{ Auth::user()->currency }}{{ number_format($total_balance, 2, '.', ',') }}</div>
                </div>
                <div class="h-1 w-full bg-gradient-to-r from-gray-800 via-emerald-500/50 to-gray-800 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            </div>
        </div>
        ...
        <!-- Total Withdrawals -->
        <div class="stats-card group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-800 hover:border-emerald-500/50 transition-all duration-300" style="animation-delay: 0.3s;">
            <div class="absolute inset-0 bg-gradient-to-br from-emerald-500/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <div class="absolute -inset-1 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10"></div>
                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative p-3 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 rounded-xl border border-emerald-500/30 backdrop-blur-sm">
                            ...
                            <svg class="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 13l5-5m0 0l5 5m-5-5v12m-7-4h14"/>
                            </svg>
                        </div>
                    </div>
                </div>
                <div class="space-y-1">
                    <div class="text-white text-2xl font-medium transition-all duration-300 group-hover:text-emerald-50" data-value="0">{{ Auth::user()->currency }}{{ number_format($total_withdrawal, 2, '.', ',') }}</div>
                </div>
                <div class="h-1 w-full bg-gradient-to-r from-gray-800 via-emerald-500/50 to-gray-800 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            </div>
        </div>
        ...
        <div class="space-y-6 lg:col-span-8">
            ...
            <div class="p-4 bg-dark-200 sm:p-6 rounded-xl">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('trading-bots.history') }}" class="text-sm font-medium text-primary-500 hover:text-primary-400">View All</a>
                </div>
                <div class="space-y-4">
                    @forelse ($plans as $userBot)
                        <div class="flex items-center justify-between p-4 bg-dark-100 rounded-lg">
                            <div class="flex items-center">
                                <div class="flex items-center justify-center w-10 h-10 text-primary-400 bg-primary-500/10 rounded-full">
                                    <i class="fas fa-robot"></i>
                                </div>
                                <div class="ml-4">
                                    <p class="font-semibold text-white">{{ $userBot->tradingBot->name }}</p>
                                    <p class="text-sm text-gray-400">Amount: {{ Auth::user()->currency }}{{ number_format($userBot->amount) }}</p>
                                </div>
                            </div>
                            <div class="hidden text-right sm:block">
                                <p class="text-sm font-medium text-white">{{ \Carbon\Carbon::parse($userBot->end_date)->toFormattedDateString() }}</p>
                            ...
                            <div class="text-right">
                                ...
                                @else
                                    <span class="px-2 py-1 text-xs font-medium text-red-400 bg-red-500/10 rounded-full">{{ ucfirst($userBot->status) }}</span>
                                @endif
                            </div>
                            <a href="{{ route('trading-bots.details', $userBot->id) }}" class="p-2 text-gray-400 rounded-md hover:bg-dark-300">
                    ...
                    @empty
                        <div class="py-8 text-center">
                            ...
                            <a href="{{ route('trading-bots.index') }}" class="px-5 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700">Start Trading Bot</a>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="p-4 bg-dark-200 sm:p-6 rounded-xl">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('copy-trading.history') }}" class="text-sm font-medium text-primary-500 hover:text-primary-400">View All</a>
                </div>
                <div class="space-y-4">
                    @foreach ($activeCopySubscriptions as $subscription)
                        <div class="flex items-center justify-between p-4 bg-dark-100 rounded-lg">
                            <div class="flex items-center">
                                <div class="relative">
                                    ...
                                    @if($subscription->expertTrader->isOnline())
                                        <span class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-dark-100 rounded-full"></span>
                                    @endif
                                </div>
                                <div class="ml-4">
                                    <p class="font-semibold text-white">{{ $subscription->expertTrader->name }}</p>
                                    <p class="text-sm text-gray-400">Amount: {{ Auth::user()->currency }}{{ number_format($subscription->amount) }}</p>
                                </div>
                            </div>
                            <div class="hidden text-right sm:block">
                                <p class="text-sm font-medium text-white">{{ \Carbon\Carbon::parse($subscription->end_date)->toFormattedDateString() }}</p>
                                <p class="text-xs text-gray-400">End Date</p>
                            </div>
                            <div class="text-right">
                            ...
                            <a href="{{ route('copy-trading.details', $subscription->id) }}" class="p-2 text-gray-400 rounded-md hover:bg-dark-300">
                                <i class="fas fa-chevron-right"></i>
                            </a>
                        </div>
                    @endforeach
        ...
        <div class="space-y-6 lg:col-span-4">
            ...
            <div class="p-4 bg-dark-200 sm:p-6 rounded-xl">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('copy-trading.index') }}" class="text-sm font-medium text-primary-500 hover:text-primary-400">View All</a>
                </div>
                <div class="space-y-3">
                    @foreach($topExperts as $expert)
                        <a href="{{ route('copy-trading.index') }}" class="block p-3 transition-colors bg-dark-100 rounded-lg hover:bg-dark-300">
                            <div class="flex items-center">
                                <div class="relative">
            ...
            <div class="p-4 bg-dark-200 sm:p-6 rounded-xl">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <a href="{{ route('deposits') }}" class="flex flex-col items-center justify-center p-4 text-center bg-dark-100 rounded-lg hover:bg-dark-300 transition-colors">
                        <div class="flex items-center justify-center w-12 h-12 mb-2 text-primary-400 bg-primary-500/10 rounded-full">
                            <i class="fas fa-download"></i>
                        </div>
                        <p class="text-sm font-medium text-white">Deposit</p>
                    </a>
                    <a href="{{ route('withdrawals') }}" class="flex flex-col items-center justify-center p-4 text-center bg-dark-100 rounded-lg hover:bg-dark-300 transition-colors">
                        <div class="flex items-center justify-center w-12 h-12 mb-2 text-primary-400 bg-primary-500/10 rounded-full">
                            <i class="fas fa-upload"></i>
                        </div>
                        <p class="text-sm font-medium text-white">Withdraw</p>
                    </a>
                    <a href="{{ route('connect-wallet') }}" class="flex flex-col items-center justify-center p-4 text-center bg-dark-100 rounded-lg hover:bg-dark-300 transition-colors">
                        <div class="flex items-center justify-center w-12 h-12 mb-2 text-primary-400 bg-primary-500/10 rounded-full">
                            <i class="fas fa-wallet"></i>
                        </div>
                        <p class="text-sm font-medium text-white">Connect Wallet</p>
                    </a>
                    <a href="{{ route('trading-bots.index') }}" class="flex flex-col items-center justify-center p-4 text-center bg-dark-100 rounded-lg hover:bg-dark-300 transition-colors">
                        <div class="flex items-center justify-center w-12 h-12 mb-2 text-primary-400 bg-primary-500/10 rounded-full">
                            <i class="fas fa-robot"></i>
                        </div>
    ...
    <div class="mt-6">
        <div class="p-4 bg-dark-200 sm:p-6 rounded-xl">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('accounthistory') }}" class="text-sm font-medium text-primary-500 hover:text-primary-400">View All</a>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-dark-100">
                    <thead class="bg-dark-100">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-400 uppercase">Type</th>
                        ...
                        @forelse ($t_history as $history)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex items-center justify-center w-8 h-8 mr-3 rounded-full {{ $history->type == 'Deposit' ? 'bg-primary-500/10 text-primary-400' : 'bg-red-500/10 text-red-400' }}">
                                            <i class="fas {{ $history->type == 'Deposit' ? 'fa-arrow-down' : 'fa-arrow-up' }}"></i>
                                        </div>
                                        <span class="font-medium text-white">{{ $history->type }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-white whitespace-nowrap">{{ Auth::user()->currency }}{{ number_format($history->amount) }}</td>
                                <td class="px-6 py-4 text-sm text-gray-400 whitespace-nowrap">{{ $history->created_at->toFormattedDateString() }}</td>
                                <!-- <td class="px-6 py-4 whitespace-nowrap">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var options = {
                ...
                yaxis: {
                    labels: {
                        style: { colors: '#9ca3af' },
                        formatter: (value) => { return `{{Auth::user()->currency}}${value.toLocaleString()}` }
                    }
                },
                tooltip: {
                    theme: 'dark',
